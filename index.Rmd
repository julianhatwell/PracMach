...
In this assigment, the objective is simply to use a training data partition to create model that can correctly predict the classification for a small test set of 20 observations, derived from the same original data set.  
...
# Do this by adding the absolute value of the minimum plus 6 standard devs for each variable.
# This is to minimise risk of new data containing a lower value. 
# These values are kept for also applying to the validation and test sets.
...
# I am creating 6 models in total, just showing the first two below:
...
First, it has to be run through the same transformations then confusion matrices are produced to analyse each model's accuracy against the validation set.
...
In the future it may not be necessary to also split the data into a training and validation set as these techniques continue to improve.
...
xyplot(Accuracy~buildTimes
       ...
       , key = list(title = "Cross Validated Performance"
                    , cex = 0.5
                    , columns = 2
                    , text = list(as.character(range(cvAccuracy)), cex = 0.5)
...
Quadratic Linear Discrimiation was the poorest performer overall, with very poor results when using the PCA transformed set, though it took a fraction of a second to build and could be useful for non-critical applications.
